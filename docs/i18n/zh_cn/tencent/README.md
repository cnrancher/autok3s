...
![](../../../assets/tencent/quick-start-tencent.png)
...
| Name               | K3s集群名称                                      |
| Tencent Credential | 云提供商的认证信息，包括 Access Key/Secret Key 访问密钥      |
...
![](../../../assets/tencent/custom-create-cluster-credential-tencent.png)
...
如果您没有配置过腾讯云认证信息，可以点击 Create Credential 按钮，输入 Access Key/Secret Key保存即可。
选择认证信息后，可以点击 Validate Credentials 按钮验证认证信息是否有效。UI 集成了 Tencent SDK，可以与腾讯云进行联动，方便用户配置实例信息，这里需要输入云凭证信息进行合法性校验，用以获取后续的实例参数。

#### 实例配置

实例配置主要配置的内容为对应云提供商虚拟机的配置，例如实例运行区域、地区、使用的操作系统类型、实例规格、网络配置等信息。

![](../../../assets/tencent/custom-create-cluster-instance-tencent.png)
...
| VPC                        | VPC ID(如果您不设置，AutoK3s 将为您创建名称为 autok3s-tencent-vpc 的VPC)                                                                                                  |
| Subnet                     | 子网 ID(如果您不设置，AutoK3s会自动在 autok3s-tencent-vpc 下创建默认子网)                                                                                                     |
...
![](../../../assets/tencent/custom-create-cluster-additional-tencent.png)
...
![](../../../assets/tencent/launch-kube-explorer.png)
...
myk3s  cn-hangzhou  alibaba   Running  2        2        v1.19.5+k3s2  true      Embedded DB(etcd)
